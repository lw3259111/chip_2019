...
class CustomizedBert(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier1 = nn.Linear(config.hidden_size + self.num_features, self.num_labels)
        self.classifier2 = nn.Linear(config.hidden_size, self.num_categories)
        self.features_bn = nn.BatchNorm1d(self.num_features)
        self.features_dense = nn.Linear(self.num_features, self.num_features)

        self.init_weights()

    def forward(self, input_ids, ct_clf_input_ids, token_type_ids=None, attention_mask=None, position_ids=None, labels=None,
                ct_clf_token_type_ids=None, ct_clf_attention_mask=None, ct_clf_position_ids=None, categories=None,
                head_mask=None, hand_features=None):
        outputs1 = self.bert(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask)
        outputs2 = self.bert(ct_clf_input_ids, position_ids=ct_clf_position_ids, token_type_ids=ct_clf_token_type_ids,
                             attention_mask=ct_clf_attention_mask, head_mask=head_mask)
        pooled_output1 = outputs1[1]
        pooled_output2 = outputs2[1]

        hand_features = hand_features.float()
        hand_features = self.features_dense(self.features_bn(hand_features))
        pooled_output1 = torch.cat((pooled_output1, hand_features), dim=1)
        ...
        outputs1 = (logits1,) + outputs1[2:]  # add hidden states and attention if they are here
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss_fct = MSELoss()
                loss = loss_fct(logits1.view(-1), labels.view(-1))
            else:
                loss_fct = CrossEntropyLoss()
                loss = loss_fct(logits1.view(-1, self.num_labels), labels.view(-1))
            outputs1 = (loss,) + outputs1
        if categories is not None:
            if self.num_categories == 1:
                #  We are doing regression
                loss_fct = MSELoss()
                loss = loss_fct(logits2.view(-1), categories.view(-1))
            else:
                loss_fct = CrossEntropyLoss()
                loss = loss_fct(logits2.view(-1, self.num_categories), categories.view(-1))
            outputs2 = (loss,) + outputs2

        return outputs1, outputs2  # (loss), logits, (hidden_states), (attentions)
...
def train(args, train_dataset, model, tokenizer):
    ...
    scheduler = Warmup(optimizer, warmup_steps=args.warmup_steps, t_total=t_total)
    if args.fp16:
        try:
...
def predict(args, model, tokenizer, index):
    ...
    preds = np.argmax(preds, axis=1)
    with open(os.path.join(args.data_dir, 'result.csv'), 'w') as f:
        f.write('id,label\n')
...
def main():
    ...
    parser.add_argument('--num_features', default=10, type=int,
                        help='Number of hand-crafted features.')
    ...
    category_list = processor.get_categories()
    clf_num_labels = len(category_list)
    ...
    if args.do_predict:
        ...
        # For bagging.
        num_test = 50000
        all = pd.DataFrame({'id': [i for i in range(num_test)], 'label': num_test*[0]})
        for i in range(10):
            args.data_dir = parent_data_dir + str(i)
            df = pd.read_csv(args.data_dir + '/result.csv')
            all['label'] += df['label']
